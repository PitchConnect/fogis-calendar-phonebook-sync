...
class SubscriptionResult:
    ...
    def __post_init__(self):
        """Initialize timestamp after dataclass creation."""
...
class CalendarServiceRedisSubscriber:
    ...
    def __init__(
        self,
        redis_config: RedisSubscriptionConfig = None,
        ...
        logger.info("📡 Calendar Service Redis Subscriber initialized")
    ...
    def _handle_redis_message(self, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if result.success:
                ...
                logger.debug(
                    f"✅ Message processed successfully in {result.processing_time_ms:.2f}ms"
                )
            else:
            ...
            "subscription_stats": {
                **self.subscription_stats,
                "subscription_start_time": (
                    self.subscription_stats["subscription_start_time"].isoformat()
                    if self.subscription_stats["subscription_start_time"]
                    else None
                ),
                "last_message_time": (
                    self.subscription_stats["last_message_time"].isoformat()
                    if self.subscription_stats["last_message_time"]
                    else None
                ),
    ...
    def test_subscription(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_message = {
                ...
                "version": "1.0",
                "type": "match_updates",
                "payload": {"matches": [], "metadata": {"has_changes": False}},
            }
...
def create_calendar_redis_subscriber(
    redis_url: str = None, calendar_sync_callback: Callable[[List[Dict]], bool] = None
