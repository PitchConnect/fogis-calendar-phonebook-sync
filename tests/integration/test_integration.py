...
def test_end_to_end_sync(setup_test_environment):
    ...
    temp_dir, config, matches = setup_test_environment
    ...
    with patch.object(fogis_calendar_sync, "build_calendar_service", return_value=mock_calendar_service), \
         patch.object(fogis_contacts, "build_people_service", return_value=mock_people_service), \
         patch.object(fogis_calendar_sync, "sys"), \
         patch.object(fogis_contacts, "sys"), \
         patch.object(fogis_calendar_sync, "logging"), \
         patch.object(fogis_contacts, "logging"), \
         patch("os.path.exists", return_value=True), \
         patch("builtins.open", side_effect=lambda f, *args, **kwargs: open(f, *args, **kwargs) if os.path.exists(f) else MagicMock()):
